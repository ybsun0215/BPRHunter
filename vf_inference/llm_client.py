...
def _build_urllib3_retries() -> UR3Retry:
    """urllib3 2.x renamed method_whitelist -> allowed_methods; stay
    compatible with both so import never crashes on a modern install."""
    kwargs = dict(
        total=3,
        backoff_factor=1.0,
    ...
    try:
        return UR3Retry(allowed_methods=frozenset(["POST"]), **kwargs)
    except TypeError:
        return UR3Retry(method_whitelist=frozenset(["POST"]), **kwargs)


_URLLIB3_RETRIES = _build_urllib3_retries()
