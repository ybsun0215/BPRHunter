...
1. search_function(file_path, function_name):
   - Used to search for the specific implementation of a given function
     within a Smali file.
2. search_text(pattern, file_filter?):
   - Global search across ALL Smali files, editor-style: every match,
     grouped by file, with line numbers and per-file inbound-reference
     counts. Use it whenever you need the complete picture of where a
     name appears before deciding what to analyse.
3. retrieve_credential(parameter):
...
@Instructions:
1. The context contains a SEARCH INDEX for the target field: every match
   grouped by file, each file tagged with its inbound-reference count
   (how many other files invoke it) and whether it is wired to the HTTP
   client stack. Multiple implementations of the same logic may coexist
   in the corpus. FIRST decide which file is on the LIVE request path:
   - Prefer files with inbound refs > 0 or HTTP-stack wiring.
   - A self-contained class marked 'no inbound refs — possibly unused
     code' is likely bundled SDK leftovers: do NOT analyse it unless no
     live path explains the field.
2. Based on the context, infer the next action to perform:
   ...
   - If a broader view of where a name appears is needed, use
     search_text(pattern).
...
or, while still exploring (arguments INLINE in next_action):
{
  "analysis_reasoning": "<what you know so far and what is missing>",
  "next_action": "search_function('com/example/sdk/util/HttpUtils.smali', 'urlEncode')"
}
or:
{
  "analysis_reasoning": "...",
  "next_action": "retrieve_credential('secretKey')"
}

@Input:
Target Field: ${target_field}$
Search Index: every match grouped by file with liveness tags
Entry-point candidates: ${source_functions}$
"""
...
def run_inference_with_codegen(
    ...
    initial_context = _build_initial_context(ctx)
...
def _inference_loop(
    ...
    for iteration in range(max_iterations):
        ...
        result, raw = _llm_infer_next_action(ctx, chat_history)
        reasoning = result.get("analysis_reasoning", "")
        action = str(result.get("next_action") or "").strip()

        # Malformed reply (non-JSON output, or JSON without next_action):
        # correct the model and let it retry instead of silently ending
        # the inference. Bounded so a hopeless model cannot loop forever.
        if "_parse_error" in result or not action:
            parse_failures += 1
            chat_history.append({"role": "assistant", "content": raw})
            if parse_failures >= config.MAX_PARSE_RETRIES:
                print("  [!] LLM output kept failing to parse — stopping inference.")
                break
            chat_history.append({
                "role": "user",
                "content": (
                    "Your last reply could not be used. Reply with a single JSON "
                    "object containing 'analysis_reasoning' (string) and "
                    "'next_action' (string). Valid next_action values: "
                    "search_function('<file_path>', '<function_name>'), "
                    "retrieve_credential('<parameter>'), or generate_code(). "
                    "Reply again with the corrected JSON."
                ),
            })
            continue
        ...
        # The full result dict is passed so the dispatcher can recover
        # arguments the LLM placed in sibling JSON fields.
        tool_result = dispatch(action, smali_dir, credentials, result)
...
def _build_initial_context(ctx: dict) -> str:
    """Initial user message: search index + compact entry candidates."""
    return (
        f"Target Field: {ctx['target_field']}\n\n"
        f"SEARCH INDEX — every match for this field, grouped by file "
        f"(live-path candidates first):\n"
        f"{ctx.get('search_index', '(not provided)')}\n\n"
        f"Entry-point candidates (JSON):\n"
        + json.dumps(ctx.get("source_functions", [])[:5], indent=2)
        + f"\n\nAdditional Context:\n{ctx.get('extra_context', 'None')}"
    )


def _llm_infer_next_action(ctx: dict, chat_history: list[dict]) -> tuple[dict, str]:
    """Call the LLM with the full conversation history for context continuity.

    Returns (result_dict, raw_reply). result_dict carries "_parse_error"
    when the reply is not usable JSON — the caller decides how to retry.
    """
    # First call: just the initial context
    if not chat_history:
        messages = [{"role": "user", "content": _build_initial_context(ctx)}]
    else:
...
def _parse_llm_json(raw: str) -> dict | None:
    """Parse an LLM reply into a dict, tolerating code fences and
    prose wrapped around the JSON object. Returns None if unusable."""
    text = (raw or "").strip()
    ...
    # Prose before/after the JSON object: decode the first {...} block.
    start = text.find("{")
    if start != -1:
        try:
